...
case 'histinter'
   ...
   for i=1:m1
       for j=1:m2
           if (m1==m2)
               if (i>=j)
                   eval(['data1=x(' num2str(i) ',1).data;'])
                    eval(['data2=xsup(' num2str(j) ',1).data;'])
                    K(i,j) =sum(sum(min(data1,data2)));        
                    K(j,i)=K(i,j);%%%%%%Computing half
               end
           else
               eval(['data1=x(' num2str(i) ',1).data;'])
               eval(['data2=xsup(' num2str(j) ',1).data;'])
               K(i,j) =sum(sum(min(data1,data2)));        
            %K(i,j)= slhistmetric_pw(data1, data2, 'intersect');
           end
       end
   end
